...
import static com.dvd.android.xposed.lockscreenmods.LockscreenSettings.ACTION_LOCKSCREEN_SETTINGS_CHANGED;
...
import static com.dvd.android.xposed.lockscreenmods.LockscreenSettings.PREF_D2TS;
...
public class ModLockscreen {
				...
				case ACTION_PREF_LOCKSCREEN_SHORTCUT_SETTING_CHANGED:
					if (mAppBar != null) {
						if (intent.hasExtra(EXTRA_LS_SHORTCUT_SLOT)) {
							mAppBar.updateAppSlot(intent.getIntExtra(
									EXTRA_LS_SHORTCUT_SLOT, 0), intent
									.getStringExtra(EXTRA_LS_SHORTCUT_VALUE));
						}
						if (intent.hasExtra(EXTRA_LS_SAFE_LAUNCH)) {
							mAppBar.setSafeLaunchEnabled(intent
									.getBooleanExtra(EXTRA_LS_SAFE_LAUNCH,
											false));
						}
						if (intent.hasExtra(PREF_SIZE_ICON)) {
							mAppBar.setSize(intent.getIntExtra(PREF_SIZE_ICON,
									40));
						}
						if (intent.hasExtra(PREF_ANIMATIONS_ENABLED)) {
							mAppBar.setAnimations(intent.getBooleanExtra(
									PREF_ANIMATIONS_ENABLED, true));
						}
						if (intent.hasExtra(PREF_LONG_CLICK)) {
							mAppBar.setLongClick(intent.getBooleanExtra(
									PREF_LONG_CLICK, false));
						}
					}
					break;
				case ACTION_SLEEP:
	...
	public static void init(final XSharedPreferences prefs,
		...
		try {
			...
			XposedHelpers.findAndHookMethod(kgViewMediatorClass,
					setupMethodName, new XC_MethodHook() {
						...
						protected void afterHookedMethod(
							...
							intentFilter
									.addAction(ACTION_LOCKSCREEN_SETTINGS_CHANGED);
			...
			XposedHelpers.findAndHookMethod(CLASS_NOTIF_PANEL_VIEW,
					classLoader, "onTouchEvent", MotionEvent.class,
					new XC_MethodHook() {
						@Override
						protected void beforeHookedMethod(
								final MethodHookParam param) throws Throwable {
							if (prefs.getBoolean(PREF_D2TS, false)
									...
									&& (int) XposedHelpers.callMethod(
											...
											"getBarState") == 1) {
								mGestureDetector
										.onTouchEvent((MotionEvent) param.args[0]);
							}
						}
					});

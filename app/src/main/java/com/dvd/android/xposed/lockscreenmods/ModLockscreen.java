...
public class ModLockscreen {
	...
	private static BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action
					.equals(ACTION_PREF_LOCKSCREEN_SHORTCUT_SETTING_CHANGED)) {
				if (mAppBar != null) {
					...
					if (intent.hasExtra(EXTRA_LS_SAFE_LAUNCH)) {
						mAppBar.setSafeLaunchEnabled(intent
								.getBooleanExtra(EXTRA_LS_SAFE_LAUNCH, false));
					}
					...
					if (intent.hasExtra(PREF_LONG_CLICK)) {
						mAppBar.setLongClick(
								intent.getBooleanExtra(PREF_LONG_CLICK, false));
					}
				}
	...
	public static void init(final XSharedPreferences prefs,
		...
		try {
			final Class<?> kgViewMediatorClass = XposedHelpers
					.findClass(CLASS_KGVIEW_MEDIATOR, classLoader);

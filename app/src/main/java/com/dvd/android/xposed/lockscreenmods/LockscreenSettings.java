...
public class LockscreenSettings extends Activity {
	...
	public static class PrefsFragment extends PreferenceFragment
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			PreferenceCategory mPrefCatLsShortcuts = (PreferenceCategory) findPreference(
					PREF_CAT_KEY_LOCKSCREEN_SHORTCUTS);
			mPrefs = getPreferenceManager().getSharedPreferences();

			if (isPackageInstalled(getActivity(),
		...
		public void onSharedPreferenceChanged(
			...
			if (PREF_KEY_LOCKSCREEN_SHORTCUT.contains(key)) {
				intent.setAction(
						ACTION_PREF_LOCKSCREEN_SHORTCUT_SETTING_CHANGED);
				...
				intent.setAction(
						ACTION_PREF_LOCKSCREEN_SHORTCUT_SETTING_CHANGED);
				intent.putExtra(PREF_SIZE_ICON,
						sharedPreferences.getInt(key, 40));
				...
				intent.setAction(
						ACTION_PREF_LOCKSCREEN_SHORTCUT_SETTING_CHANGED);
				intent.putExtra(PREF_ANIMATIONS_ENABLED, sharedPreferences
						.getBoolean(PREF_ANIMATIONS_ENABLED, true));
			} else if (key.equals(PREF_LONG_CLICK)) {
				intent.setAction(
						ACTION_PREF_LOCKSCREEN_SHORTCUT_SETTING_CHANGED);
				intent.putExtra(PREF_LONG_CLICK,
						sharedPreferences.getBoolean(PREF_LONG_CLICK, true));
			} else if (key.equals(PREF_HIDE_ICON)) {
				int mode = sharedPreferences.getBoolean(key, false)
						? PackageManager.COMPONENT_ENABLED_STATE_DISABLED
						: PackageManager.COMPONENT_ENABLED_STATE_ENABLED;
				getActivity().getPackageManager().setComponentEnabledSetting(
						new ComponentName(getActivity(), mainClass), mode,
		...
		public void onActivityResult(int requestCode, int resultCode,
			...
			if (requestCode == 1028 && mShortcutHandler != null) {
				if (resultCode == Activity.RESULT_OK) {
					Bitmap b = null;
					Intent.ShortcutIconResource siRes = data.getParcelableExtra(
							Intent.EXTRA_SHORTCUT_ICON_RESOURCE);
					if (siRes != null) {
						try {
							final Context extContext = getActivity()
									.createPackageContext(siRes.packageName,
											Context.CONTEXT_IGNORE_SECURITY);
							final Resources extRes = extContext.getResources();
							final int drawableResId = extRes.getIdentifier(
									siRes.resourceName, "drawable",
									siRes.packageName);
							b = BitmapFactory.decodeResource(extRes,
									drawableResId);
					...
					mShortcutHandler.onHandleShortcut(
							(Intent) data.getParcelableExtra(
				...
				if (resultCode == Activity.RESULT_OK) {
					try {
						File f = new File(data.getStringExtra(
								PickImageActivity.EXTRA_FILE_PATH));

	...
	public static class PrefsFragment extends PreferenceFragment implements
		...
		public void onSharedPreferenceChanged(
			...
			if (PREF_KEY_LOCKSCREEN_SHORTCUT.contains(key)) {
				...
				int mode = sharedPreferences.getBoolean(key, false) ? PackageManager.COMPONENT_ENABLED_STATE_DISABLED
		...
		public void onActivityResult(int requestCode, int resultCode,
			...
			if (requestCode == 1028 && mShortcutHandler != null) {
				if (resultCode == Activity.RESULT_OK) {
					Bitmap b = null;
					Intent.ShortcutIconResource siRes = data
					...
					mShortcutHandler.onHandleShortcut((Intent) data
				...
				if (resultCode == Activity.RESULT_OK) {
					try {
						File f = new File(
								data.getStringExtra(PickImageActivity.EXTRA_FILE_PATH));

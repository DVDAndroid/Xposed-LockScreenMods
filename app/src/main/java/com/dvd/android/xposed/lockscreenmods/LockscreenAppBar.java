...
public class LockscreenAppBar {
	...
	public LockscreenAppBar(Context ctx, Context gbCtx, ViewGroup container,
		...
		mAnimations = prefs
				.getBoolean(LockscreenSettings.PREF_ANIMATIONS_ENABLED, true);
		mLongClick = prefs.getBoolean(LockscreenSettings.PREF_LONG_CLICK,
				false);
	...
	private void initAppSlots() {
		...
		for (int i = 0; i < LockscreenSettings.PREF_KEY_LOCKSCREEN_SHORTCUT
				.size(); i++) {
			updateAppSlot(i, mPrefs.getString(
					LockscreenSettings.PREF_KEY_LOCKSCREEN_SHORTCUT.get(i),
					null), false);
		}
	...
	private void updateAppSlot(int slot, String value,
			boolean updateRootViewVisibility) {
	...
	private void startActivity(Intent intent) {
		...
		if (ShortcutActivity.isGbBroadcastShortcut(intent)) {
			Intent newIntent = new Intent(
					intent.getStringExtra(ShortcutActivity.EXTRA_ACTION));
	...
	private final class AppInfo
		...
		public void initAppInfo(String value) {
			...
			try {
				...
				final int iconResId = mIntent
						.getStringExtra("iconResName") != null
								? mResources.getIdentifier(
										mIntent.getStringExtra("iconResName"),
										"drawable", mGbContext.getPackageName())
								: 0;
				if (iconResId != 0) {
